...
int begin=0,end=0,left=0,right=0,initial=0;
int i,j=1,k;
int terminated[100] = {0};
int time = 30, timeten, timeone, count = 0;
int score = 0,scoreten = 0,scoreone = 0;
...
int32_t main (void)
{
	...
	while(1)
	{
		...
		{	
			...
			for (y = loc ; y < loc + 8 ; y ++) 
			{
				draw(3, y, numabc[timeten][y-loc]);
			}	
			loc=11;
			for (y = loc ; y < loc + 8 ; y ++) 
			{
				draw(3, y, numabc[timeone][y-loc]);
			}
