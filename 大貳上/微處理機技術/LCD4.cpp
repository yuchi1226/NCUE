...
extern  SPI_T * SPI_PORT[4]={SPI0, SPI3, SPI2, SPI3};
	...
	uint8_t  in1, in2, in11, in22;
	uint8_t  result;
	uint8_t  loc;
	uint8_t y;
	int first = 0, second = 0, press11 = 0, press22 = 0;
	int ten = 0, hex = 1;
	int tens = 0, ones = 0;
		...
		if(first == 0)
		{
			if(hex == 1)
			{
				...
				{		
					...
					tGPIO_A -> DOUT = act[1];
					u32Reg_temp = inpw(u32Reg);
					if((u32Reg_temp & 0x08) == 0) //A
					{
						DrvSYS_Delay(1000000000);
						num1 = 10;
						first = 1;		
					}
					else if((u32Reg_temp & 0x10) == 0) //C
					{
						DrvSYS_Delay(1000000000);
						num1 = 12;
						first = 1;		
					}	
					else if((u32Reg_temp & 0x10) == 0) //E
					{
						DrvSYS_Delay(1000000000);
						num1 = 14;
						first = 1;		
					...
					tGPIO_A -> DOUT = act[2];
					u32Reg_temp = inpw(u32Reg);
					if((u32Reg_temp & 0x08) == 0) //D
					{
						DrvSYS_Delay(1000000000);
						num1 = 13;
						first = 1;		
					}
					else if((u32Reg_temp & 0x20) == 0) //F
					{
						DrvSYS_Delay(1000000000);
						num1 = 15;
						first = 1;		
